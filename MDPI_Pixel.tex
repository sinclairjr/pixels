...
\abstract{
	Traditional charge readout technologies of single-phase Liquid Argon Time projection Chambers (LArTPCs) based on projective wire readout introduce intrinsic ambiguities in event reconstruction, combined with a slow detector response, these ambiguities have limited the performance of LArTPCs, until now.
...
While generally applicable to any particle type, as a proof of concept we assumed a minimum-ionising muon with an initial momentum of \SI{260}{\mega\electronvolt} in the direction of the track estimate from the PCA.
