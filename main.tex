...
\begin{figure}[htb]
	...
	\caption{Reconstructed 3D hits from the hit finder.
		...
		Each hit is represented by a sphere of a radius proportional to the pulse height.
		This results in a droplet shape because the radius decreases when the ionisation track moves towards a neighbouring pixel, depositing less charge on the current pixel over time.}
	\label{fig:pca}
\end{figure}
